...
def render_pdf_bytes_to_base64png(pdf_bytes: bytes, page_num: int, target_longest_image_dim: int = 2048) -> str:
    # Load the PDF from bytes
    doc = fitz.open(stream=pdf_bytes, filetype="pdf")
    assert 0 <= page_num - 1 < len(doc), "Invalid page number"

    # Get the page and render it to a pixmap
    page = doc[page_num - 1]
    pix = page.get_pixmap(matrix=fitz.Matrix(2, 2))  # default zoom 2x
...
def prepare_ocr_messages(
    ...
    filename_byte = pdf_or_image_path
    ...
    try:
        ...
        image_base64 = render_pdf_to_base64png(
            filename_byte, page_num, target_longest_image_dim=target_image_dim
        )
